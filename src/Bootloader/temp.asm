...
LoadSectors:
    ...
    jne DiskError           ; Jump if not equal (error)

    ret                     ; Return

DiskError:
    ; Handle disk error here
    jmp Failure             ; Jump to failure handling
...
.loop:
    ...
    pop di                  ; Restore DI
    je LoadFAT              ; Jump if match found
    pop cx                  ; Restore CX
...
.load_image_loop:
    ...
    mov bx, ROOTDIRECTORY_AND_FAT_OFFSET   ; FAT memory location
    add bx, cx              ; Index into FAT
    mov dx, word [bx]       ; Read FAT entry
